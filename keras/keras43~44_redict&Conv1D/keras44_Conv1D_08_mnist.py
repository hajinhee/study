...
#model.add(LSTM(10,return_sequences=False,activation='relu'))                                  # 윗줄을 주석하고 input shape 넣지않고 바로 실행해도 알아서 모델이 돌아갑니다.
...
model.fit(x_train,y_train, epochs=20, batch_size=5000,validation_split=0.2,verbose=1)        # batch_size 센스껏 조절  ,callbacks=[es]
