...
#scaler = RobustScaler()
#scaler = MaxAbsScaler()
scaler.fit(x_train)  # 어떤 비율로 변환할지 계산(여기서 구한 비율로 transform)
x_train = scaler.transform(x_train)  # train_set 변환  
x_test = scaler.transform(x_test)    # test_set 변환 
# 설령 스케일링 밖의 값을 받아도 이미 weight구했으므로 예측값이나온다.
##################################### 스케일러 설정 옵션 ########################################
